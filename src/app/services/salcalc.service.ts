...
import {EMPS} from '../models/mock-emps';
...
export class SalcalcService {
  ...
  constructor(private http:HttpClient, private srvMsge:MessageService) { }
  ...
  getEvalSal2(obj:{}):Observable<{}>{

    return this.http.post(this.evalSalUrl_self,obj,this.httpOptions);
  }

  getEvalSalInitConfig():Observable<{}>{

    return this.http.post<{}>(
      this.evalSalUrl_self,
      {'action':'init'},
      this.httpOptions)
...
private log(message: string) {
  this.srvMsge.add(`empService with msg -> ${message}`);
}
